...
const animateSprite = ({ 
  ...
  // 투명도 (나타났다가 사라지기)
  gsap.timeline().to(sprite.material, {
      opacity: 1,
      duration: 0.3,
      ease: "power1.in"
  }).to(sprite.material, {
...
const TailEffect: React.FC = () => {
  ...
  const createCloudEffect = () => {
    ...
    animateSprite({
        ...
        duration: EFFECTS.CLOUD.DURATION.MIN + Math.random() * (EFFECTS.CLOUD.DURATION.MAX - EFFECTS.CLOUD.DURATION.MIN),
        onComplete: () => {
            if (sprite.parent) {
...
animateSprite({
    ...
    duration: EFFECTS.SPARKLE.DURATION.MIN + Math.random() * (EFFECTS.SPARKLE.DURATION.MAX - EFFECTS.SPARKLE.DURATION.MIN),
    onComplete: () => {
        if (sprite.parent) {
            sprite.parent.remove(sprite);
        }
